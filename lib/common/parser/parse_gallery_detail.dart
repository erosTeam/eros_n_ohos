...
/// (ideally off the critical render path) to fill in translated names and
/// enrich related-gallery simpleTags from the local DB.
...
/// from the local DB. Should be called after the page is already rendered
/// so it doesn't block the initial display.
///
...
Future<Gallery> enrichGalleryDetail(Gallery raw) async {
  ...
  // Passive learning: fire-and-forget, must not block the caller.
  unawaited(
...
/// `simpleTags` of every related gallery using batch DB queries.
Future<Gallery> _enrichGalleryDetail(Gallery raw) async {
  // Enrich top-level tags: batch by namespace to stay precise.
  final enrichedTags = await _enrichDetailTags(raw.tags);

  // Enrich related galleries' simpleTags via the shared batch helper.
...
Future<List<Tag>> _enrichDetailTags(List<Tag> tags) async {
  ...
  // One batch query per distinct namespace (typically ≤ 6).
