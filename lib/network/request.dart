...
///
/// 1. POST `/api/v2/galleries/{gid}/download` → returns `{ download_url }`.
...
Future<String> downloadTorrent({
  ...
  // Query param `format` chooses zip (default), cbz, or torrent.
  ...
  // Use a callback savePath so we can read content-disposition for the real
  // filename, falling back to "{gid}.torrent".
